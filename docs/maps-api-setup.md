...
- The secrets store `config/secrets.json` is also git-ignored. Provision `JWT_SECRET` before token use; missing or blank values fail closed. An example structure is provided in `config/secrets.example.json`.
