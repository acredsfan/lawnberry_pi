...
| --- | --- | --- | --- |
| RoboHAT USB control bridge | **Stable enough for active maintenance** | `backend/src/services/robohat_service.py` now speaks the RP2040 text protocol, probes serial ports, maintains USB control, accepts the firmware's Python-dict status payloads, and marks the controller ready as soon as the firmware acknowledges `rc=disable`. Focused regression coverage lives in `tests/unit/test_robohat_service_usb_control.py`. | Still hardware-sensitive by nature; treat serial timing, RC takeover, and emergency-stop behavior as regression-sensitive. |
