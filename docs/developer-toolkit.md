...
| Navigation | **Partial implementation with recent safety hardening** | `backend/src/services/navigation_service.py` has path planning hooks, waypoint driving, return-home behavior, weather gating, and contract coverage such as `tests/contract/test_waypoint_navigation.py`. At each mission start a GPS COG heading bootstrap drives briefly forward to derive a `session_heading_alignment`; adjusted heading is `(-raw_yaw + imu_yaw_offset_degrees + session_heading_alignment) % 360`. Saved alignment is now only used as an initial seed and a **fresh bootstrap snap is always required** before alignment is treated as verified for the mission. Bootstrap also rejects low-confidence snaps (extreme delta or high COG spread) instead of silently committing them and now fails closed unless preflight GPS accuracy is RTK-grade (<= 0.25 m). `imu_yaw_offset_degrees` remains a physical mounting offset only, not a GPS-derived heading fix. GPS antenna/body-center offsets are configurable via `gps.antenna_offset_forward_m` and `gps.antenna_offset_right_m`; fixed map imagery display nudges are separate and display-only. Waypoint completion now requires a fresh, non-dead-reckoned GPS fix instead of advancing on stale synthetic position data, and live mission traversal now reuses the shared ToF clearance threshold from `config/limits.yaml` instead of a separate 1 m autonomy-only block. | Obstacle handling is still simplistic, dead reckoning still uses placeholder distance estimates rather than field-grade odometry feedback, and waypoint traversal still needs more real-world validation around heading quality and obstacle interpretation. |
...
| AI perception | **Real runtime; model qualification required** | `backend/src/services/detector_runtime.py` loads a configured ONNX detector through OpenCV DNN. The sole camera owner publishes exact-frame, timestamped results with model SHA-256 provenance; FastAPI validates them before WebSocket/UI delivery and before adding short-lived semantic route costs. | No model artifact is bundled. Missing config, OpenCV, or model stays explicitly unavailable. Semantic detections can only increase planner cost and never override ToF, geometry, localization, qualification, or `MotorCommandGateway`. Model quality and distance calibration require physical validation. |
...
3. **Qualify onboard perception without contract churn**
	- train and field-qualify the private ONNX artifact behind the stable provenance/freshness surface
	- add accelerators later only if they improve measured latency and reliability rather than just complexity
