...
Generated on 2026-03-18. This document summarizes code files, their purpose, subsystem category, and callable interfaces (public functions and exported APIs). Grouped by subsystem for quick orientation.
...
| `backend/src/api/rest.py` | Compatibility REST router for map setup, manual-control, emergency-stop, lightweight planning jobs, docs-root compatibility, and other stateful API surfaces used by the frontend and contract tests. | API/Core | Helpers: `_docs_root()`, `_normalize_map_provider(provider) -> str`, `_default_map_configuration_envelope(config_id: str) -> dict[str, Any]`, `_geometry_conflicts(zones: list[dict[str, Any]], zone_types?) -> list[str]`, `_legacy_polygon_zones(entries, zone_type) -> list[dict[str, Any]]`. Endpoints: `GET/PUT /api/v2/map/configuration`, `POST /api/v2/map/provider-fallback`, `GET/POST /api/v2/map/zones`, `GET/PUT /api/v2/map/locations`, `GET/POST /api/v2/planning/jobs`, `DELETE /api/v2/planning/jobs/{job_id}`, `POST /api/v2/control/start|pause|resume|stop|return-home`, `GET /api/v2/control/status`, `POST /api/v2/control/drive|blade|emergency|emergency-stop`, `GET /api/v2/hardware/robohat`. |
...
| `frontend/src/services/websocket.ts` | WebSocket service with fallback URL probing, topic subscriptions, connection-state listeners, heartbeat, and composable wrappers for telemetry/control channels. | Frontend realtime | Interface `WebSocketMessage`. Class `WebSocketService(url: string)` with `connect()`, `subscribe(topic: string)`, `unsubscribe(topic: string)`, `onTopic(topic: string, callback)`, `offTopic(topic: string, callback?)`, `setCadence(cadenceHz: number)`, `ping()`, `listTopics()`, `disconnect()`, `onConnectionStateChange(callback)`, `dispatchTestMessage(message: WebSocketMessage)`. Exported factory: `useWebSocket(type: 'telemetry' | 'control' = 'telemetry', handlers?: { onMessage?: (msg: any) => void })`. |
