...
| `backend/src/services/mission_service.py` | Mission execution and lifecycle management; validates waypoints, maps lifecycle state, and integrates with `NavigationService`. | Missions | Class `MissionService`: `create_mission(name, waypoints) -> Mission`, `start_mission(mission_id)`, `pause_mission(mission_id)`, `resume_mission(mission_id)`, `abort_mission(mission_id)`, `get_mission_status(mission_id) -> MissionStatus`, `list_missions() -> list[Mission]`. Module-level: `get_mission_service(nav_service: NavigationService | None = None) -> MissionService`. |
