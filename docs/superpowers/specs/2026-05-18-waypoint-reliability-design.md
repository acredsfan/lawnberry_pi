...
## Fix 2 — RTK-tiered waypoint tolerance + per-waypoint field
...
The mower is ~550mm long × ~300mm wide (half-width = 0.15m). It has an RTK-capable GPS (u-blox F9P) whose driver already populates `Position.accuracy` with RTK-aware values: ~0.03m for RTK Fixed, ~0.20m for RTK Float, and HDOP-derived values (~0.5–2.0m) for standard GPS. Tolerance must scale with the actual GPS precision to enable geometric stripe patterns when RTK is active and degrade gracefully without it.

### Tiered tolerance

Add `_tiered_waypoint_tolerance() -> float` to `MissionExecutor`. It reads `current_position.accuracy` and returns:

| accuracy | tolerance | rationale |
|----------|-----------|-----------|
| ≤ 0.05m (RTK Fixed) | **0.15m** | Mower half-width; cm GPS supports this |
| ≤ 0.25m (RTK Float) | **0.30m** | Full mower width; float is ~0.20m |
| ≤ 1.0m (standard GPS) | **0.65m** | ~2× half-width for metre-class GPS |
| > 1.0m or `None` | `self.waypoint_tolerance` (default **1.0m**) | No-data fallback |

The static `waypoint_tolerance` parameter (previously 0.5m) changes default to **1.0m** and serves only as the no-data fallback. All callers use `_tiered_waypoint_tolerance()` instead of `self.waypoint_tolerance` directly for arrival and decel calculations.

### Per-waypoint override

Add `arrival_threshold_m: float | None = None` to `MissionWaypoint`. When set, it bypasses the tier entirely — `go_to_waypoint` uses it directly. This allows boundary waypoints (e.g., last point before a garden bed) to lock to a specific tight value regardless of GPS quality.

All existing missions that omit the field continue to work unchanged (None → tier lookup).

### Tests

- Unit: `_tiered_waypoint_tolerance()` returns 0.15m when accuracy=0.03m (RTK Fixed)
- Unit: returns 0.30m when accuracy=0.20m (RTK Float)
- Unit: returns 0.65m when accuracy=0.80m (standard GPS)
- Unit: returns `self.waypoint_tolerance` (1.0m) when accuracy=None
- Unit: per-waypoint `arrival_threshold_m=0.10m` bypasses tier
- Unit: per-waypoint `arrival_threshold_m=None` uses tier
...
When `distance_to_target < decel_start_distance` (= 3 × `_tiered_waypoint_tolerance()`), replace flat `cruise_speed` with:
...
- `decel_start_distance = 3 × _tiered_waypoint_tolerance()` (recomputed each tick — adapts if GPS quality changes during approach)

At 0.15 m/s and 5 Hz, the mower travels 0.03m per tick — ≥ 3 ticks within even the tightest 0.15m RTK-Fixed zone. Zero chance of slip-through.
...
|------|-----------|-----------------|
| **Full confidence** | position exists, not dead-reckoning, fix age < 2.0s, accuracy within threshold | Check distance vs `_tiered_waypoint_tolerance()` |
| **Degraded confidence** | position exists, fix age < 5.0s, but `accuracy=None` or marginally stale | Check distance vs `2 × _tiered_waypoint_tolerance()`; log `WARNING: waypoint arrival in degraded GPS mode` |
...
- Unit: arrival check uses `2 × _tiered_waypoint_tolerance()` in degraded mode, `_tiered_waypoint_tolerance()` in full mode
...
| RTK-tiered tolerance + per-waypoint field | `system_configuration.py`, `navigation_service.py`, `mission_executor.py`, `mission.py` | Yes — default 1.0m fallback | Yes — `_tiered_waypoint_tolerance()` method |
