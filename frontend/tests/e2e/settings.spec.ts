...
test.describe('System settings persistence', () => {
  ...
  test('persists system settings after saving', async ({ page }) => {
    ...
    await timezoneSelect.selectOption('US/Eastern')
    ...
    expect(saveCall?.body).toMatchObject({ timezone: 'US/Eastern', debug_mode: true })

    await page.reload()
    await expect(page.getByLabel('Timezone')).toHaveValue('US/Eastern')
