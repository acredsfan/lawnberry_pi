<template>
  <div class="planning-view">
    ...
    <div v-if="activeTab === 'current'" class="tab-content">
      <div class="card">
        <div class="card-header">
          ...
          <button class="btn btn-sm btn-secondary" @click="refreshJobs">🔄 Refresh</button>
        </div>
        <div class="card-body">
          <div v-if="jobs.length === 0" class="empty-state">
            <p>No active or queued jobs</p>
            <button class="btn btn-primary" @click="openScheduleModal">Schedule First Job</button>
          </div>

          <div v-else class="jobs-list">
            <div v-for="job in jobs" :key="job.id" class="job-item" :class="`status-${job.status}`">
              ...
              <div class="job-details">
                <div class="job-info">
                  ...
                  <span v-if="job.scheduled_start"
                    >Start: {{ formatDateTime(job.scheduled_start) }}</span
                  >
                </div>
    ...
    <div v-if="activeTab === 'schedule'" class="tab-content">
      <div class="card">
        <div class="card-header">
          ...
          <button class="btn btn-sm btn-primary" @click="openScheduleModal">➕ Add Schedule</button>
        </div>
        <div class="card-body">
              ...
              <div class="zone-stats">
                ...
                <div class="stat">
                  ...
                  <span class="stat-value"
                    >{{ formatCuttingHeight(zone.cutting_height) }} {{ cuttingHeightUnit }}</span
                  >
                </div>
                <div class="stat">
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <div class="zone-checkboxes">
              <label v-for="zone in zones" :key="zone.id" class="checkbox-label">
                <input v-model="scheduleForm.zones" type="checkbox" :value="zone.id" />
                {{ zone.name }}
              </label>
            </div>
          </div>
          ...
          <div v-if="scheduleForm.type === 'recurring'" class="recurring-options">
            ...
            <div class="form-group">
              ...
              <input v-model="scheduleForm.timeOfDay" type="time" class="form-control" />
            </div>
          </div>
        </div>
    ...
    <div
      v-if="statusMessage"
      class="alert"
      :class="statusSuccess ? 'alert-success' : 'alert-danger'"
    >
      {{ statusMessage }}
    </div>
  </div>
...
<script setup lang="ts">
  ...
  async function startQuickMow() {
    try {
      ...
      if (job.status !== 'running') {
        throw new Error(`Server returned ${job.status}`)
      }
      showStatus('Quick mow accepted and running', true)
      await refreshJobs()
    } catch (error) {
      showStatus('Quick mow was not accepted by the mower', false)
    }
  }
  ...
  async function startJob(job: any) {
    try {
      const updated = await planningStore.startJob(String(job.id))
      if (updated.status !== 'running') throw new Error(`Server returned ${updated.status}`)
      showStatus('Job accepted and running', true)
    } catch (error) {
      showStatus('Job start was not accepted', false)
    }
  }

  async function pauseJob(job: any) {
    try {
      const updated = await planningStore.pauseJob(String(job.id))
      if (updated.status !== 'paused') throw new Error(`Server returned ${updated.status}`)
      showStatus('Job pause confirmed', true)
    } catch (error) {
      showStatus('Job pause was not confirmed', false)
    }
  }

  async function resumeJob(job: any) {
    try {
      const updated = await planningStore.resumeJob(String(job.id))
      if (updated.status !== 'running') throw new Error(`Server returned ${updated.status}`)
      showStatus('Job resume confirmed', true)
    } catch (error) {
      showStatus('Job resume was not confirmed', false)
    }
  }
  ...
  async function cancelJob(job: any) {
    ...
    try {
      const updated = await planningStore.cancelJob(String(job.id))
      if (updated.status !== 'cancelled') throw new Error(`Server returned ${updated.status}`)
      showStatus('Job cancellation confirmed', true)
    } catch (error) {
      showStatus('Job cancellation was not confirmed', false)
    }
  }
  ...
  async function toggleSchedule(schedule: any) {
    try {
      ...
      showStatus(schedule.enabled ? 'Schedule disabled' : 'Schedule enabled', true)
    } catch (error) {
  ...
  async function mowZone(zone: DisplayZone) {
    try {
      ...
      if (job.status !== 'running') throw new Error(`Server returned ${job.status}`)
      showStatus(`Mowing ${zone.name} accepted and running`, true)
      await refreshJobs()
    } catch (error) {
      showStatus(`Mowing ${zone.name} was not accepted`, false)
    }
  }
