...
from .models import DataType, OperationalState, SensorReading
...
class CacheManager:
        ...
        try:
            ...
            if not REDIS_AVAILABLE or not self.redis_client:
                ...
                if cached_data is not None:
                    ...
                    if response_time > 10:
                        self.logger.warning(
                            f"Cache get (mem) took {response_time:.2f}ms for key {cache_key}"
                        )
                    return cached_data
                else:
                    self.stats["misses"] += 1
    ...
    async def set(
        self, data_type: DataType, key: str, value: Any, ttl: Optional[int] = None
    ) -> bool:
    ...
    async def add_to_stream(
        self, stream_name: str, data: Dict[str, Any], max_length: int = 1000
    ) -> bool:
        ...
        try:
            ...
            await self.redis_client.xadd(stream_name, data, maxlen=max_length, approximate=True)
            return True
    ...
    async def read_stream(
        self, stream_name: str, count: int = 100, start: str = "-"
    ) -> List[Dict[str, Any]]:
