...
async def get_model_performance(
    model_name: str, system_manager=Depends(get_system_manager)
) -> Dict[str, Any]:
...
async def test_detection(system_manager=Depends(get_system_manager)) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "data": test_results, "message": "Detection test completed"}
...
async def update_vision_config(
    config_update: Dict[str, Any], system_manager=Depends(get_system_manager)
) -> Dict[str, Any]:
...
async def get_camera_status(system_manager=Depends(get_system_manager)) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "data": {
                "available": True,
                "capturing": camera_manager._capturing
                if hasattr(camera_manager, "_capturing")
                else False,
