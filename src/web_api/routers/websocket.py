...
async def handle_websocket_message(
    websocket: WebSocket, message: Dict[str, Any], mqtt_bridge: MQTTBridge
):
    ...
    try:
        if message_type == "subscribe":
            ...
            await ws_manager.send_personal_message(
                {"type": "status", "data": status_data, "timestamp": datetime.utcnow().isoformat()},
                websocket,
            )
...
async def broadcast_message(message: Dict[str, Any], topic: str = None):
