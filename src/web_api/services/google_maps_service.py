...
class GoogleMapsService:
        ...
        try:
            async with self.session.get(url, params=params) as response:
                if response.status == 200:
                    ...
                    raise HTTPException(status_code=response.status, detail="Google Maps API request failed")
    ...
    async def _update_usage_stats(self, api_type: str, cost: float = 0.0):
        ...
        try:
            ...
            await self.redis_client.setex(
                stats_key,
                self.cache_config.usage_stats_ttl,
                json.dumps(stats.to_dict())
            )
            
            # Check cost alert
            if stats.cost_estimate > self.cost_alert_threshold:
                logger.warning(f"Google Maps API cost alert: ${stats.cost_estimate:.2f} (threshold: ${self.cost_alert_threshold})")
    ...
    async def reverse_geocode(self, lat: float, lng: float) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            if data.get('results'):
                ...
                # Update usage stats
                await self._update_usage_stats('reverse_geocoding', self.PRICING['reverse_geocoding'] / 1000)
    ...
    async def search_places(self, query: str, location: Optional[Tuple[float, float]] = None, radius: int = 5000) -> List[Dict[str, Any]]:
        """
        ...
        cache_key = await self._get_cache_key("places", query=query, location=location, radius=radius)
