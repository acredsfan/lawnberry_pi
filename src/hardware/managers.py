...
from .exceptions import (
    ...
    DeviceBusyError, DeviceTimeoutError
)
from .data_structures import (
    SensorReading, I2CDeviceReading, SerialDeviceReading, 
    GPIOReading, CameraFrame, RoboHATStatus, DeviceHealth
)
...
class I2CManager:
    ...
    async def read_register(self, address: int, register: int, 
    ...
    async def write_register(self, address: int, register: int, 
                           data: int | List[int]):
        ...
        async with self.device_access(device_name) as conn:
            for attempt in range(self._retry_policy.max_retries + 1):
                try:
                    ...
                    if attempt == self._retry_policy.max_retries:
                        raise CommunicationError(
                            f"Failed to write to {device_name}: {e}"
                        )
...
class GPIOManager:
    ...
    async def setup_pin(self, pin: int, direction: str, 
                       pull_up_down: str = 'none', initial: int = 0):
        """Setup GPIO pin"""
...
class CameraManager:
    ...
    async def initialize(self, width: int = 1920, height: int = 1080, fps: int = 30):
        ...
        async with self._lock:
            try:
                self._camera = cv2.VideoCapture(self.device_path)
