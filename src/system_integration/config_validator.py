...
class ConfigValidator:
        ...
        return {
            "system": {
                ...
                "field_validators": {
                    ...
                    "data_directory": {"type": "path", "must_exist": True, "permissions": "rw"},
                    "config_directory": {"type": "path", "must_exist": True, "permissions": "r"},
                    "max_memory_usage_mb": {
                        "type": "int",
                        "min": 512,
                        "max": 7900,  # For 8GB system (leave headroom below 8192)
                    },
                    "max_cpu_usage_percent": {"type": "float", "min": 10.0, "max": 95.0},
                },
            },
            "hardware": {
                "required_fields": [
                ...
                "field_validators": {
                    ...
                    "i2c_devices": {"type": "dict", "address_range": (0x03, 0x77)},
                    "uart_devices": {"type": "dict", "device_pattern": r"^/dev/tty[A-Z]+\d+$"},
                    "camera_device": {"type": "string", "pattern": r"^/dev/video\d+$"},
                    "update_rate_hz": {"type": "int", "min": 1, "max": 100},
                },
            },
            "safety": {
                "required_fields": [
                ...
                "field_validators": {
                    "emergency_stop_enabled": {"type": "bool", "must_be": True},
                    "obstacle_detection_enabled": {"type": "bool", "must_be": True},
                    "max_tilt_angle": {"type": "float", "min": 5.0, "max": 45.0},
                    "min_battery_voltage": {"type": "float", "min": 10.0, "max": 15.0},
                    "safety_boundaries": {"type": "list", "min_items": 3},  # Minimum triangle
                },
            },
            "network": {
                "required_fields": ["web_api_port", "websocket_port"],
                "field_validators": {
                    "web_api_port": {"type": "int", "min": 1024, "max": 65535},
                    "websocket_port": {"type": "int", "min": 1024, "max": 65535},
                    "mqtt_broker_host": {"type": "hostname_or_ip", "optional": True},
                    "mqtt_broker_port": {"type": "int", "min": 1, "max": 65535, "optional": True},
                },
            },
            "weather": {
                "required_fields": ["api_key", "location"],
                "field_validators": {
                    "api_key": {"type": "string", "min_length": 32, "pattern": r"^[a-f0-9]+$"},
                    "location": {"type": "dict", "required_keys": ["latitude", "longitude"]},
                    "update_interval": {
                        "type": "int",
    ...
    async def validate_configuration(
        self, config_name: str, config_data: Dict[str, Any], config_path: Optional[Path] = None
    ) -> ValidationSummary:
        ...
        logger.info(
            f"Configuration validation completed: {error_count} errors, {warning_count} warnings"
        )

        return summary

    async def _validate_syntax(
        self, config_name: str, config_data: Dict[str, Any], config_path: Optional[Path]
    ...
    async def _validate_field(
        self, field_path: str, value: Any, validator: Dict[str, Any]
    ) -> List[ValidationResult]:
    ...
    async def _validate_path(
        self, field_path: str, value: str, validator: Dict[str, Any]
    ) -> List[ValidationResult]:
            ...
            if not path.exists():
                results.append(
                    ValidationResult(
                        ...
                        field_path=field_path,
                        current_value=value,
                        fix_command=f"mkdir -p {path.parent}"
                        if validator.get("create_if_missing")
                        else None,
                    )
                )
    ...
    async def _validate_security(
        ...
        for field_path, value in self._flatten_dict(config_data):
            if isinstance(value, str):
                ...
                if any(
                    keyword in field_path.lower()
                    for keyword in ["key", "password", "secret", "token"]
                ):
    ...
    async def _validate_i2c_addresses(self, i2c_config: Dict[str, Any]) -> List[ValidationResult]:
        ...
        for device_name, address in i2c_config.items():
            ...
            if (
                address in common_devices
                and device_name.lower() not in common_devices[address].lower()
            ):
                results.append(
                    ValidationResult(
                        level=ValidationLevel.WARNING,
    ...
    async def _validate_cross_configuration(
    ...
    async def export_validation_report(
        self, validation_summary: ValidationSummary, format: str = "json"
    ) -> str:
