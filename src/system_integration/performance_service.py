...
class PerformanceTimer:
    ...
    def __init__(self, performance_service, metric_name: str, category: PerformanceCategory, tags: Dict[str, str] = None):
...
class PerformanceService:
    ...
    async def _collect_system_metrics(self):
        ...
        try:
            ...
            await self.record_metric("cpu_usage", PerformanceCategory.CPU, MetricType.GAUGE, cpu_percent, {}, "%")
            await self.record_metric("cpu_count", PerformanceCategory.CPU, MetricType.GAUGE, cpu_count, {}, "cores")
            await self.record_metric("load_avg_1m", PerformanceCategory.CPU, MetricType.GAUGE, load_avg[0], {}, "")
            
            # Memory metrics
            memory = psutil.virtual_memory()
            swap = psutil.swap_memory()
            
            await self.record_metric("memory_usage", PerformanceCategory.MEMORY, MetricType.GAUGE, memory.percent, {}, "%")
            await self.record_metric("memory_used_mb", PerformanceCategory.MEMORY, MetricType.GAUGE, memory.used / 1024 / 1024, {}, "MB")
            await self.record_metric("memory_available_mb", PerformanceCategory.MEMORY, MetricType.GAUGE, memory.available / 1024 / 1024, {}, "MB")
            await self.record_metric("swap_usage", PerformanceCategory.MEMORY, MetricType.GAUGE, swap.percent, {}, "%")
            ...
            await self.record_metric("disk_usage", PerformanceCategory.DISK_IO, MetricType.GAUGE, disk_percent, {}, "%")
            
            if disk_io:
                await self.record_metric("disk_read_mb_s", PerformanceCategory.DISK_IO, MetricType.GAUGE, 
                                       disk_io.read_bytes / 1024 / 1024, {}, "MB/s")
                await self.record_metric("disk_write_mb_s", PerformanceCategory.DISK_IO, MetricType.GAUGE, 
                                       disk_io.write_bytes / 1024 / 1024, {}, "MB/s")
            
            # Network metrics
            network_io = psutil.net_io_counters()
            if network_io:
                await self.record_metric("network_sent_mb_s", PerformanceCategory.NETWORK, MetricType.GAUGE,
                                       network_io.bytes_sent / 1024 / 1024, {}, "MB/s")
                await self.record_metric("network_recv_mb_s", PerformanceCategory.NETWORK, MetricType.GAUGE,
                                       network_io.bytes_recv / 1024 / 1024, {}, "MB/s")
            
            # Process-specific metrics
            current_process = psutil.Process()
            await self.record_metric("process_cpu_percent", PerformanceCategory.CPU, MetricType.GAUGE,
                                   current_process.cpu_percent(), {"process": "lawnberry"}, "%")
            
            memory_info = current_process.memory_info()
            await self.record_metric("process_memory_mb", PerformanceCategory.MEMORY, MetricType.GAUGE,
                                   memory_info.rss / 1024 / 1024, {"process": "lawnberry"}, "MB")
    ...
    async def _collect_service_metrics(self):
        ...
        try:
            ...
            if gc_stats:
                await self.record_metric("gc_collections", PerformanceCategory.MEMORY, MetricType.COUNTER,
                                       sum(stat['collections'] for stat in gc_stats), {}, "count")
            
            # Thread count
            thread_count = threading.active_count()
            await self.record_metric("thread_count", PerformanceCategory.CPU, MetricType.GAUGE,
                                   thread_count, {}, "count")
            
            # Open file descriptors
            try:
                fd_count = len(psutil.Process().open_files())
                await self.record_metric("open_files", PerformanceCategory.DISK_IO, MetricType.GAUGE,
                                       fd_count, {}, "count")
    ...
    async def record_metric(self, name: str, category: PerformanceCategory, metric_type: MetricType, 
                          value: float, tags: Dict[str, str] = None, unit: str = "", description: str = ""):
                ...
                if metric_key in current_metrics:
                    ...
                    if benchmark.higher_is_better:
                        if current_value < benchmark.threshold_critical:
                            await self._trigger_performance_alert(benchmark, current_value, "critical")
                        elif current_value < benchmark.threshold_warning:
                            await self._trigger_performance_alert(benchmark, current_value, "warning")
                    else:
                        if current_value > benchmark.threshold_critical:
                            await self._trigger_performance_alert(benchmark, current_value, "critical")
                        elif current_value > benchmark.threshold_warning:
    ...
    async def _trigger_performance_alert(self, benchmark: PerformanceBenchmark, current_value: float, severity: str):
            ...
            for rule_name, rule in self.optimization_rules.items():
                ...
                try:
                    ...
                    context.update({
                        ...
                        'api_response_time': current_metrics.get('web_api.api_response_time', 0),
    ...
    async def _generate_performance_report(self):
        ...
        try:
            ...
            overall_score, category_scores = await self._calculate_performance_scores(start_time, end_time)
            ...
            self.logger.info(f"Generated performance report {report_id} - Score: {overall_score:.1f}/100")
        
        except Exception as e:
            self.logger.error(f"Failed to generate performance report: {e}")
    
    async def _calculate_performance_scores(self, start_time: datetime, end_time: datetime) -> Tuple[float, Dict[PerformanceCategory, float]]:
        """Calculate performance scores for the given time period"""
        category_scores = {}
        
        # Calculate scores for each category
        for category in PerformanceCategory:
            category_scores[category] = await self._calculate_category_score(category, start_time, end_time)
    ...
    async def _calculate_category_score(self, category: PerformanceCategory, start_time: datetime, end_time: datetime) -> float:
    ...
    def _calculate_metric_score(self, value: float, benchmark: PerformanceBenchmark) -> float:
        ...
        if benchmark.higher_is_better:
            if value >= benchmark.target_value:
                ...
                return 70.0 + (value - benchmark.threshold_warning) / (benchmark.target_value - benchmark.threshold_warning) * 30.0
            elif value >= benchmark.threshold_critical:
                return 30.0 + (value - benchmark.threshold_critical) / (benchmark.threshold_warning - benchmark.threshold_critical) * 40.0
            else:
            ...
            if value <= benchmark.target_value:
                ...
                return 70.0 + (benchmark.threshold_warning - value) / (benchmark.threshold_warning - benchmark.target_value) * 30.0
            elif value <= benchmark.threshold_critical:
                return 30.0 + (benchmark.threshold_critical - value) / (benchmark.threshold_critical - benchmark.threshold_warning) * 40.0
            else:
                return max(0.0, 30.0 * (1.0 - min(1.0, (value - benchmark.threshold_critical) / benchmark.threshold_critical)))
    
    async def _generate_recommendations(self, category_scores: Dict[PerformanceCategory, float]) -> List[str]:
        ...
        for category, score in category_scores.items():
            if score < 70.0:
                if category == PerformanceCategory.CPU:
                    ...
                    recommendations.append("Optimize computer vision processing or reduce frame rate")
    ...
    async def _establish_baseline(self):
        ...
        try:
            ...
            self.logger.info(f"Performance baseline established with {len(self.baseline_metrics)} metrics")
    ...
    def timer(self, metric_name: str, category: PerformanceCategory, tags: Dict[str, str] = None) -> PerformanceTimer:
    ...
    async def get_performance_metrics(self, category: Optional[PerformanceCategory] = None, 
