...
class WeatherService:
    ...
    def _load_config(self) -> Dict[str, Any]:
        ...
        default_config = {
            ...
            "api_rate_limit": {"calls_per_hour": 100, "calls_per_day": 1000},
        }
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            if not self.api_key:
                ...
                self.logger.error(
                    "OpenWeather API key not configured; running in offline/test mode"
                )
                return False
    ...
    async def get_current_weather(self, force_refresh: bool = False) -> Optional[WeatherCondition]:
        ...
        if (
            not force_refresh
            and self._current_weather
            and current_time - self._last_api_call < self._cache_duration
        ):
    ...
    async def get_forecast(self, days: int = 7) -> List[WeatherForecast]:
        ...
        # Check cache
        current_time = time.time()
        if self._forecast_data and current_time - self._last_api_call < self._cache_duration:
    ...
    async def get_weather_alerts(self) -> List[WeatherAlert]:
        ...
        # Check cache first
        now = time.time()
        if self._active_alerts and now - self._last_alerts_call < self._cache_duration:
    ...
    async def evaluate_mowing_conditions(
        self, local_sensor_data: Optional[Dict] = None
    ) -> MowingConditions:
        """
    ...
    async def predict_solar_charging(self, hours_ahead: int = 24) -> List[Tuple[datetime, float]]:
        ...
        for forecast_day in forecast:
            ...
            for hour in range(24):
                ...
                if (
                    forecast_time < now
                    or (forecast_time - now).total_seconds() > hours_ahead * 3600
                ):
                    continue
    ...
    def _calculate_solar_efficiency(
        self, time: datetime, sunrise: datetime, sunset: datetime, clear_sky_percent: float
    ) -> float:
    ...
    async def _find_optimal_mowing_window(self) -> Tuple[Optional[datetime], Optional[datetime]]:
        ...
        for forecast_day in forecast:
            ...
            for hour in range(6, 20):  # Mow between 6 AM and 8 PM
                ...
                if (
                    forecast_day.precipitation_chance < 20
                    and forecast_day.temp_high < self.temp_max  # Less than 20% chance of rain
                    and forecast_day.temp_low > self.temp_min
                    and forecast_day.wind_speed < self.wind_max
                ):
    ...
    def _trim_weather_history(self):
        ...
        if len(self._weather_history) > self._max_history_days * 24:  # 24 readings per day
            ...
            self._weather_history = [w for w in self._weather_history if w.timestamp > cutoff_date]
        ...
        if history_file.exists():
            try:
                with open(history_file, "r") as f:
                    ...
                    self._weather_history = [WeatherCondition(**item) for item in history_data]
    ...
    async def get_weather_trends(self, days: int = 7) -> Dict[str, Any]:
        ...
        recent_history = [
            w for w in self._weather_history if w.timestamp > datetime.now() - timedelta(days=days)
        ]
