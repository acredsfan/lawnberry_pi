...
class MaintenanceSafetySystem:
    ...
    def __init__(self, mqtt_client: MQTTClient, access_controller: SafetyAccessController, 
        ...
        self.battery_capacity_threshold = config.get('battery_capacity_threshold', 80.0)  # Percentage
    ...
    async def _handle_vibration_data(self, data: Dict[str, Any]):
        ...
        self.vibration_history = [
            (t, v) for t, v in self.vibration_history if t > cutoff_time
        ]
    ...
    async def _analyze_blade_condition(self):
        ...
        if wear_percentage > 0:
            estimated_remaining_hours = max(0, (100 - wear_percentage) * 2)  # 2 hours per percentage point
        else:
        ...
        logger.debug(f"Blade analysis: {condition.value}, wear: {wear_percentage:.1f}%, efficiency: {cutting_efficiency:.2f}")
    ...
    async def _handle_excessive_vibration(self, vibration: float):
        ...
        diagnostic = DiagnosticResult(
            ...
            status=MaintenanceStatus.ATTENTION_NEEDED if vibration < 3.0 else MaintenanceStatus.CRITICAL,
            result_data={'vibration_level': vibration, 'threshold': self.vibration_threshold},
            issues_found=[f"Excessive vibration: {vibration:.2f}g (threshold: {self.vibration_threshold}g)"],
            recommendations=["Check blade balance", "Inspect blade for damage", "Check motor mount"],
            safety_impact=True
        )
        ...
        if vibration > 3.0:
            await self._create_mechanical_safety_lockout("excessive_vibration", 
                                                       f"Excessive vibration: {vibration:.2f}g")
    ...
    async def _create_blade_safety_lockout(self, blade_data: BladeWearData):
        ...
        lockout = MaintenanceLockout(
            ...
            expires_at=datetime.now() + timedelta(hours=24) if severity != MaintenanceStatus.CRITICAL else None
        )
    ...
    async def _create_battery_safety_lockout(self, battery_data: BatteryHealthData):
        ...
        lockout = MaintenanceLockout(
            ...
            required_access_level=SafetyAccessLevel.TECHNICIAN if "overheating" in battery_data.safety_concerns else SafetyAccessLevel.ADVANCED,
    ...
    async def _create_battery_safety_lockout_direct(self, concern: str, description: str):
    ...
    async def _run_diagnostic(self, diagnostic_type: str, user: str):
        ...
        logger.info(f"Completed {diagnostic_type} diagnostic: {len(diagnostic_tests)} tests performed")
    ...
    async def _diagnostic_battery_system(self) -> DiagnosticResult:
        ...
        for battery_id, battery_data in self.battery_data.items():
            if battery_data.capacity_percentage < 70:
                issues.append(f"Battery {battery_id} low capacity: {battery_data.capacity_percentage:.1f}%")
            ...
            if battery_data.safety_concerns:
                issues.append(f"Battery {battery_id} safety concerns: {', '.join(battery_data.safety_concerns)}")
    ...
    async def _diagnostic_mechanical_system(self) -> DiagnosticResult:
        ...
        mechanical_lockouts = [
            lockout for lockout in self.active_lockouts.values()
        ...
        return DiagnosticResult(
            ...
            result_data={
                ...
                'average_vibration': statistics.mean([v for _, v in self.vibration_history]) if self.vibration_history else 0
            },
            issues_found=issues,
            recommendations=recommendations,
            safety_impact=safety_impact
    ...
    async def _schedule_maintenance_reminder(self, maintenance_type: str, schedule: Dict[str, Any]):
        ...
        reminder = {
            ...
            'overdue_hours': (datetime.now() - schedule['last_performed']).total_seconds() / 3600 - schedule['frequency_hours'],
            'created_at': datetime.now()
        }
    ...
    async def get_maintenance_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "recent_diagnostics": len([d for d in self.diagnostic_history if (datetime.now() - d.timestamp).days < 7]),
            "system_health": "critical" if any(l.severity == MaintenanceStatus.CRITICAL for l in self.active_lockouts.values()) else "good"
        }
