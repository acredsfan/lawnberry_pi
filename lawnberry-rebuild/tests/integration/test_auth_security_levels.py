...
async def test_authentication_security_levels_enforced():
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        sec = resp.json()
        assert sec["level"] in {"password_only", "password_totp", "google_auth", "cloudflare_tunnel_auth"}

        # Update to password_totp
        put = await client.put("/api/v2/settings/security", json={"level": "password_totp", "totp_digits": 6})
        ...
        put2 = await client.put("/api/v2/settings/security", json={"level": "google_auth", "google_client_id": "id"})
