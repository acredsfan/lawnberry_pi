...
def test_backup_and_restore_roundtrip():
    ...
    with tempfile.TemporaryDirectory() as src_dir, tempfile.TemporaryDirectory() as dest_dir, tempfile.TemporaryDirectory() as restore_dir:
        src = Path(src_dir)
        (src / "config").mkdir()
        ...
        assert (r / "config" / "system.json").read_text().strip() == '{"telemetry": {"cadence_hz": 5}}'
