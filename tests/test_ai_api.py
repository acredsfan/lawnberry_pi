...
@pytest.fixture
def client(tmp_path):
    config_path = tmp_path / "ai-detector.json"
    ai_service = AIService(
        model_path=str(config_path),
        confidence_threshold=0.2,
        detector_runtime=_FakeRuntime(tmp_path / "detector.onnx"),
    )
...
def test_get_ai_status(client):
    ...
    assert response.json()["configured_model_path"].endswith("ai-detector.json")
    assert response.json()["runtime"] == "opencv_dnn"
...
def test_run_uploaded_inference(client):
    ...
    assert payload["model_sha256"] == "b" * 64
...
def test_perception_latest_exposes_fresh_provenance(client):
    test_client, _service = client
    test_client.post(
        "/api/v2/ai/inference?frame_id=frame.jpg",
        content=_make_test_image_bytes(),
    )

    response = test_client.get("/api/v2/ai/perception/latest")

    assert response.status_code == 200
    assert response.json()["fresh"] is True
    assert response.json()["result"]["input_frame_id"] == "frame.jpg"
